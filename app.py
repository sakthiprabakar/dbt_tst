...
select *
from CTE_source
qualify row_number() over (partition by <PRIMARY_KEY columns> order by <Modified date(AUDIT DATE COLUMN if present)> desc, SF_INSERT_TIMESTAMP desc) = 1
)
...
, CTE_final as (
select
{{ generate_surrogate_key(['<Primary Key columns']) }} as PK_ODS_ES_EQAI_<Table name>_ID,
...
3. Do not include SF_INSERT_TIMESTAMP column in surrogate key generation.
"""
...
if option == "Upload SQL file":
    ...
    if st.button("Generate DBT Script"):
        if uploaded_file and primary_keys:
            ...
            if script_type == "Custom DBT Script Generation":
                prompt = base_prompt + f"""Before Create a batch id and final table Using this instructions create a custom SQL query in the form of CTE : {custom_instructions}\n\n #Note : This custom query strictly generated as separate CTE only. Do not include the custom logic in default templates like Deduplication or final table. \n\n""" + prompt_template
            else:
